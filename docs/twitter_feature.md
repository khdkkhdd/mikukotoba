...
> ミク言葉 - Twitter(X) 사이트에 특화된 번역 기능의 사용자 관점 기능 명세
> 작성일: 2026-02-19
...
트위터 타임라인은 스크롤할 때 새 트윗을 계속 로드한다. ミク言葉는 새로 나타나는 요소를 자동으로 감지하여 번역한다.

### 6.2 가상 스크롤

트위터는 가상 스크롤을 사용하여 화면 밖의 요소를 DOM에서 제거하고, 다시 화면에 들어오면 재생성한다. ミク言葉는 이를 대응한다:
...
트위터는 SPA(Single Page Application)이므로 페이지 이동 시 전체 페이지가 새로 로드되지 않는다. 홈 → 프로필 → 검색 등으로 이동해도 ミク言葉가 재시작 없이 새 콘텐츠를 감지하여 번역한다.
...
| 트위터 자체 번역과 중복 가능 | 트위터의 "이 트윗 번역하기" 버튼과 ミク言葉 번역이 동시에 표시될 수 있음 |
...
| **트위터 자체 번역 충돌 방지** | 트위터의 "이 트윗 번역하기" 버튼을 숨기거나, ミク言葉가 활성화된 상태에서 중복 표시 방지 | 두 번역이 동시에 표시되면 혼란스러움 |
