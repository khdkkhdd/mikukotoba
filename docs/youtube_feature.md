...
> JP Helper - YouTube 사이트에 특화된 번역 기능의 사용자 관점 기능 명세
> 작성일: 2026-02-19
...
2. 자막이 있으면 YouTube 기본 자막을 숨기고, JP Helper 오버레이로 대체한다.
...
YouTube의 SPA(Single Page Application) 특성상, 추천 영상 클릭 등으로 영상이 전환되어도 페이지가 새로 로드되지 않는다. JP Helper는 이를 감지하여:
...
- ON으로 전환하면 다시 JP Helper 오버레이가 나타난다.
...
YouTube 시청 페이지의 영상 설명은 기본적으로 접혀 있다. "더보기"를 클릭하여 설명을 펼치면 JP Helper가 이를 감지하고 번역을 시작한다.

### 3.5 스크롤 대응

YouTube는 댓글, 추천 영상 등을 스크롤 시 동적으로 로드한다. JP Helper는 이를 감지하여:
